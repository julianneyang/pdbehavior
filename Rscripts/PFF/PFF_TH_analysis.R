...
## PFF Synucleinopathy-- 
data <- readr::read_csv(here("Analysis_Files/PFF/PFF Brain Staining and Imaging Record - Analysis_Ch_TH(1).csv"))
data$Genotype<-factor(data$Genotype,levels=c("WT","HET","MUT"))
names(data)
striatum <- data %>% filter(Exclude=="No")
...
df <- df %>%
  group_by(Merged_ID) %>%
  mutate(Mean_Background = Count[Category == "Ipsilateral"] / Count[Category == "Contralateral"]) %>%
  ungroup()
