...
all_p <- lm(Average_Latency ~ Day+ Weight + SLC_Genotype + Sex, data =  subset(data, Treatment == "MPTP"))
summary(all_p)
all_n <- lm(Average_Latency ~ Day+ Weight + SLC_Genotype + Sex, data =  subset(data, Treatment == "Negative"))
summary(all_n)
lm_day1_p <- lm(Average_Latency ~  Weight + SLC_Genotype + Sex, data = subset(data, Day == "One" & Treatment == "MPTP"))
summary(lm_day1_p)
lm_day1_n <- lm(Average_Latency ~  Weight + SLC_Genotype + Sex, data = subset(data, Day == "One" & Treatment == "Negative"))
summary(lm_day1_n)
lm_day2_p <- lm(Average_Latency ~  Weight + SLC_Genotype + Sex , data = subset(data, Day == "Two" & Treatment == "MPTP"))
summary(lm_day2_p)
lm_day2_n <- lm(Average_Latency ~  Weight + SLC_Genotype + Sex , data = subset(data, Day == "Two" & Treatment == "Negative"))
summary(lm_day2_n)
lm_day3_p <- lm(Average_Latency ~ Weight + SLC_Genotype + Sex , data = subset(data, Day == "Three" & Treatment == "MPTP"))
summary(lm_day3_p)
lm_day3_n <- lm(Average_Latency ~ Weight + SLC_Genotype + Sex , data = subset(data, Day == "Three" & Treatment == "Negative"))
summary(lm_day3_n)

lm_day1_p_f <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "One" & Treatment == "MPTP" & Sex =="Female"))
summary(lm_day1_p_f)
lm_day1_n_f <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "One" & Treatment == "Negative" & Sex == "Female"))
summary(lm_day1_n_f)
lm_day2_p_f <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "Two" & Treatment == "MPTP" & Sex =="Female"))
summary(lm_day2_p_f)
lm_day2_n_f <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "Two" & Treatment == "Negative" & Sex == "Female"))
summary(lm_day2_n_f)
lm_day3_p_f <- lm(Average_Latency ~ Weight + SLC_Genotype, data = subset(data, Day == "Three" & Treatment == "MPTP" & Sex == "Female"))
summary(lm_day3_p_f)
lm_day3_n_f <- lm(Average_Latency ~ Weight + SLC_Genotype, data = subset(data, Day == "Three" & Treatment == "Negative" & Sex == "Female"))
summary(lm_day3_n_f)

lm_day1_p_m <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "One" & Treatment == "MPTP" & Sex =="Male"))
summary(lm_day1_p_m)
lm_day1_n_m <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "One" & Treatment == "Negative" & Sex == "Male"))
summary(lm_day1_n_m)
lm_day2_p_m <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "Two" & Treatment == "MPTP" & Sex =="Male"))
summary(lm_day2_p_m)
lm_day2_n_m <- lm(Average_Latency ~  Weight + SLC_Genotype, data = subset(data, Day == "Two" & Treatment == "Negative" & Sex == "Male"))
summary(lm_day2_n_m)
lm_day3_p_m <- lm(Average_Latency ~ Weight + SLC_Genotype, data = subset(data, Day == "Three" & Treatment == "MPTP" & Sex == "Male"))
summary(lm_day3_p_m)
...
lme_model_p <- lme(Average_Latency ~ SLC_Genotype*Day + Weight + SLC_Genotype + Sex, 
                 random = ~ 1 | MouseID, 
                 data = subset(data,Treatment =="MPTP"))
summary(lme_model_p)
